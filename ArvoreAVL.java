...
public class ArvoreAVL<C extends Comparable<C>> implements
		ArvoreBinaria<C> {
	C elem;
	ArvoreAVL<C> esq;
	ArvoreAVL<C> dir;
	...
	public boolean metodoEquals(ArvoreAVL<C> arvoreA){
	...
	public void insere(C elemento) {
		if (this.elem == null) {
			this.elem = elemento;
			this.esq = new ArvoreAVL<C>();
			this.dir = new ArvoreAVL<C>();
	...
	public ArvoreAVL<C> retornaArvoreDireita(C elemento) {
			if(this.elem.compareTo(elemento)<0)
	...
	public ArvoreAVL<C> retornaArvoreEsquerda(C elemento) {
		if(this.elem.compareTo(elemento)>0)
	...
	public int fatorB(){
		return esq.retornaAltura() - dir.retornaAltura(); 
	...
	private PilhaArray<ArvoreAVL<C>> pilha = new PilhaArray<ArvoreAVL<C>>();
        private int cont;
        ...
        public C next() {
            if (hasNext()){
            	cont++;
            	if(!pilha.estaVazia()){
            		ArvoreAVL<C> retornado = pilha.desempilha(); 
	...
	protected class IteratorPosOrdem implements Iterator<C>{
		...
		public C next() {
        	...
        	while(desempilhado instanceof ArvoreBinaria){
            	...
            	if (!((ArvoreAVL<C>)desempilhado).dir.estaVazia())
            		pilha.empilha(((ArvoreAVL<C>)desempilhado).dir);
            	if (!((ArvoreAVL<C>)desempilhado).esq.estaVazia())
            		pilha.empilha(((ArvoreAVL<C>)desempilhado).esq);
	...
	protected class IteratorInOrdem implements Iterator<C>{
		...
		public C next() {
        	...
        	while(desempilhado instanceof ArvoreBinaria){
            	...
            	pilha.empilha((((ArvoreAVL<C>)desempilhado).retornaRaiz()));
